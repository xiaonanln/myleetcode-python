...
class Solution(object):
	def fourSum(self, nums, target):
		...
		"""
		nums.sort()
		N2 = [(n+nums[j], i, j) for i, n in enumerate(nums) for j in xrange(i+1, len(nums))]
		N2.sort()
		i = 0; j = len(N2) - 1
		sols = []
		while i < j:
			...
			# 	continue 
			v = n1 + n2
			if v < target: # let v be larger 
				i += 1
			elif v > target: # let v be smaller
				j -= 1
			else:
				oi = i; oj = j
				while i+1 < j and N2[i+1][0] == n1: i += 1 # skip equals
				while i+1 < j and N2[j-1][0] == n2: j -= 1 # skip equals

				for _i in xrange(oi, i+1):
					for _j in xrange(oj, j-1, -1):
						_, _i1, _j1 = N2[_i]
						_, _i2, _j2 = N2[_j]
						if (_i1 < _j1 < _i2 < _j2):
							sols.append( [nums[i1], nums[j1], nums[i2], nums[j2]] )

				if N2[i+1][0] == n1:
					i += 1
				elif N2[j-1][0] == n2:
					j -= 1
				else: # i+1 > i and j-1 < j
					i += 1
					j -= 1
...
print Solution().fourSum([0, 0, 0], 0)
